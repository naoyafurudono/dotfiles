...
function mise
  if test (count $argv) -eq 0
    command /Users/naoya-furudono/.local/bin/mise
    return
  end
  ...
  if contains -- --help $argv
    command /Users/naoya-furudono/.local/bin/mise "$command" $argv
    return $status
  end
  ...
  case deactivate shell sh
    ...
    if contains -- -h $argv
      ...
      source (command /Users/naoya-furudono/.local/bin/mise "$command" $argv |psub)
    end
  case '*'
    command /Users/naoya-furudono/.local/bin/mise "$command" $argv
  end
...
function __mise_env_eval --on-event fish_prompt --description 'Update mise environment when changing directories';
    /Users/naoya-furudono/.local/bin/mise hook-env -s fish | source;
...
function fish_command_not_found
    if string match -qrv -- '^(?:mise$|mise-)' $argv[1] &&
        /Users/naoya-furudono/.local/bin/mise hook-not-found -s fish -- $argv[1]
        /Users/naoya-furudono/.local/bin/mise hook-env -s fish | source
