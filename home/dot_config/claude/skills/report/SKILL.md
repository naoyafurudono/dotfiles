...
1. **タイトル・著者・日付** — 著者はClaudeをファーストオーサー、ユーザーをセカンドオーサーとする（`\author{Claude\thanks{Anthropic} \and ユーザー名}`）
...
6. **参考文献** — `thebibliography` 環境で記述し、本文中から `\cite` で参照する。`hyperref` によりPDF内でクリッカブルなリンクになる
